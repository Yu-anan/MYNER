...
def train_base(opt, train_examples, dev_examples):
    ...
    if dev_examples is not None:
        ...
        # print('dev_features111111',dev_features[0].labels)
        ...
        # print('dev_features222222',dev_dataset[0]['labels'])
        eval_sampler = SequentialSampler(dev_dataset)
        dev_loader = DataLoader(dev_dataset, sampler=eval_sampler,batch_size=64,
...
def training(opt):
    ...
    train_raw_examples = processor.readfile(os.path.join(opt.raw_data_dir, 'valid.txt'))
    ...
    dev_examples = None
    if opt.eval_model:
        dev_raw_examples = processor.readfile(os.path.join(opt.raw_data_dir, 'valid.txt'))
...
def stacking(opt):
    ...
    stack_raw_examples = processor.readfile(os.path.join(opt.raw_data_dir, 'stack.txt'))
    # print('stack_raw_examples',len(stack_raw_examples))
    # pseudo_raw_examples = processor.read_json(os.path.join(opt.raw_data_dir, 'pseudo.json'))
    ...
    for i, (train_ids, dev_ids) in enumerate(kf.split(stack_raw_examples)):
        ...
        # print('train_examples',len(train_raw_examples))
        # add pseudo data to train data
        #train_raw_examples = train_raw_examples + pseudo_raw_examples
